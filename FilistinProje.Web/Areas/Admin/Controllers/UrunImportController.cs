    ...
    public class UrunImportController : AdminBaseController
    {
        ...
        public async Task<IActionResult> Duzelt()
        {
            ...
            foreach (var sec in secenekler)
            {
                ...
                {
                    sec.CerceveTipi = "AhÅŸap Ã‡erÃ§eve";
                    degisti = true;
                }
            ...
            return Json(new { success = true, message = duzeltilen + " kayÄ±t dÃ¼zeltildi" });
        }
        ...
        public async Task<IActionResult> DeleteAllProducts([FromBody] DeleteProductsRequest request)
        {
            try
            {
                if (request.kategoriIds != null && request.kategoriIds.Any())
                {
                    ...
                    if (!urunler.Any())
                    {
                        return Json(new { success = true, message = "Silinecek Ã¼rÃ¼n bulunamadÄ±" });
                    }
                    ...
                    return Json(new { success = true, message = urunler.Count + " Ã¼rÃ¼n, " + secenekler.Count + " varyasyon, " + resimler.Count + " resim, " + sepetItems.Count + " sepet, " + yorumlar.Count + " yorum, " + favoriler.Count + " favori silindi" });
                }
        ...
        public async Task<IActionResult> ImportJson([FromBody] ImportJsonRequest request)
        {
            try
            {
                ...
                {
                    return Json(new { success = false, message = "Dosya bulunamadÄ±: " + jsonPath });
                }
                ...
                if (kategori == null)
                {
                    ...
                    kategori = new Kategori
                    {
                        ...
                        KisaAciklama = "",
                        Aciklama = "",
                        SeoTitle = request.categoryName + " - 7ANRPS48",
                ...
                return Json(new { 
                    success = true, 
                    message = $"âœ… {eklenen} Ã¼rÃ¼n eklendi, {varyasyonEklendi} varyasyon eklendi, {mevcutAtlandi} Ã¼rÃ¼n atlandÄ±" 
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> ImportAtaturk()
        {
            try
            {
                var jsonPath = GetImportJsonPath("AtatÃ¼rk ve TÃ¼rkiye");
        ...
        public async Task<IActionResult> ImportDini()
        {
            try
            {
                var jsonPath = GetImportJsonPath("Dini ve Hat SanatÄ±");
        ...
        private string NormalizeCerceveTipi(string? cerceve)
        {
            ...
            if (cerceve.Equals("cercevesiz", StringComparison.OrdinalIgnoreCase) ||
                cerceve.Equals("cerÃ§evesiz", StringComparison.OrdinalIgnoreCase))
            {
                return "Ã‡erÃ§evesiz";
            }

            if (cerceve.Equals("ahsap cerceve", StringComparison.OrdinalIgnoreCase) ||
                cerceve.Equals("ahÅŸap Ã§erÃ§eve", StringComparison.OrdinalIgnoreCase))
            {
                return "AhÅŸap Ã‡erÃ§eve";
            }

            if (cerceve.Equals("metal cerceve", StringComparison.OrdinalIgnoreCase))
            {
                return "Metal Ã‡erÃ§eve";
            }
