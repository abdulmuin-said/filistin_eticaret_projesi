    ...
    public class HomeController : AdminBaseController
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var citySales = siparisler
                ...
                .Select(g => new DashboardCitySalesItem
                {
                    Sehir = g.Key,
                    SiparisAdedi = g.Count(),
                    Ciro = g.Where(IsRevenueOrder).Sum(x => x.ToplamTutar)
                })
                .OrderByDescending(x => x.Ciro)
                .ThenByDescending(x => x.SiparisAdedi)
                .Take(8)
                .ToList();
            ...
            var monthlyLabels = onikiAy
                .Select(x => x.ToString("MMM yyyy"))
                .ToList();

            var monthlyValues = onikiAy
                .Select(monthStart =>
                {
                    var monthEnd = monthStart.AddMonths(1);
                    return odenmisSiparisler
                        .Where(x => x.OlusturulmaTarihi >= monthStart && x.OlusturulmaTarihi < monthEnd)
                        .Sum(x => x.ToplamTutar);
                })
                .ToList();
            ...
            var previousMonthRevenue = odenmisSiparisler
                .Where(x => x.OlusturulmaTarihi >= gecenAyBaslangic && x.OlusturulmaTarihi < gecenAyBitis)
                .Sum(x => x.ToplamTutar);
