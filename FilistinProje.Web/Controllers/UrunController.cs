    ...
    public class UrunController : Controller
    {
            ...
            [FromQuery(Name = "sayfa")] int? eskiPage = null)
        {
            ...
            var mevcutMarkalar = await query
                ...
                .ThenBy(x => x.Marka)
                .ToListAsync();
            ViewBag.MarkaListesi = mevcutMarkalar;
            ...
            var maxFiyat = await query.MaxAsync(x => (decimal?)(x.IndirimliFiyat ?? x.Fiyat));
            ViewBag.AvailableMinFiyat = Math.Floor(minFiyat ?? 0);
            ViewBag.AvailableMaxFiyat = Math.Ceiling(maxFiyat ?? 10000);
        ...
        private string NormalizeFeatureValueLabel(string? value)
        {
            return value switch
            {
                "true" => _localizer["Urun_Yes"].Value,
                "false" => _localizer["Urun_No"].Value,
