    ...
    public class SepetController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var tamamlayiciUrunler = await _context.Urunler
                ...
                .Where(u => u.AktifMi && u.YayindaMi && !u.SilindiMi && !sepettekiUrunIdleri.Contains(u.Id))
        ...
        public async Task<IActionResult> KuponUygula(string kuponKodu)
        {
            if (string.IsNullOrEmpty(kuponKodu))
            {
                TempData["SepetHata"] = _localizer["Sepet_EnterCouponCode"].Value;
            ...
            if (kupon == null)
            {
                TempData["SepetHata"] = _localizer["Sepet_InvalidCoupon"].Value;
                return RedirectToAction("Index");
            }

            if (!kupon.AktifMi)
            {
                TempData["SepetHata"] = _localizer["Sepet_CouponNotActive"].Value;
                return RedirectToAction("Index");
            }

            if (kupon.SonKullanmaTarihi < DateTime.UtcNow)
            {
                TempData["SepetHata"] = _localizer["Sepet_CouponExpired"].Value;
                return RedirectToAction("Index");
            }

            if (kupon.KullanimLimiti > 0 && kupon.KullanilanMiktar >= kupon.KullanimLimiti)
            {
                TempData["SepetHata"] = _localizer["Sepet_CouponUsageLimitExceeded"].Value;
                return RedirectToAction("Index");
            }
            ...
            {
                TempData["SepetHata"] = string.Format(_localizer["Sepet_CouponMinCartNotMet"].Value, kupon.MinSepetTutari.ToString("C"));
                return RedirectToAction("Index");
            }
        ...
        public async Task<IActionResult> Ekle(int UrunId, int? SecenekId, int Adet = 1, string? CerceveModeli = null, string? MusteriNotu = null, decimal? CerceveFarki = null, bool HediyePaketi = false, decimal? HediyePaketFiyati = null)
        {
            ...
            if (success)
            {
                return Json(new { success = true, message = _localizer["Sepet_ProductAdded"].Value, redirectUrl = Url.Action("Index", "Sepet") });
            }
            else
            {
                return Json(new { success = false, message = _localizer["Sepet_ProductAddFailed"].Value });
            }
        }
        ...
        public async Task<IActionResult> Temizle()
        {
            ...
            {
                HttpContext.Session.Remove("UygulananKupon");
                TempData["SepetBasari"] = _localizer["Sepet_CartCleared"].Value;
            }
            else
            {
                TempData["SepetHata"] = _localizer["Sepet_CartClearFailed"].Value;
            }
