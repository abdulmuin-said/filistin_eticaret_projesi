...
BEGIN
    ...
    CREATE TABLE IF NOT EXISTS "UrunHediyePaketSecenekleri" (
        "Id" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        "UrunId" integer NULL,
        "Ad" character varying(150) NOT NULL,
        "AdEn" character varying(150) NOT NULL,
        "AdAr" character varying(150) NOT NULL,
        "Fiyat" numeric(18,2) NOT NULL,
        "AktifMi" boolean NOT NULL DEFAULT true,
        "Sira" integer NOT NULL DEFAULT 0,
        "OlusturulmaTarihi" timestamp with time zone NOT NULL DEFAULT NOW(),
        "SilindiMi" boolean NOT NULL DEFAULT false,
        CONSTRAINT "FK_UrunHediyePaketSecenekleri_Urunler_UrunId" FOREIGN KEY ("UrunId") REFERENCES "Urunler" ("Id") ON DELETE CASCADE
    );

    ALTER TABLE "UrunHediyePaketSecenekleri" ALTER COLUMN "UrunId" DROP NOT NULL;

    ALTER TABLE "SepetItems" ADD COLUMN IF NOT EXISTS "HediyePaketSecenegiId" integer NULL;
    ALTER TABLE "SepetItems" ADD COLUMN IF NOT EXISTS "HediyePaketAdi" character varying(150) NOT NULL DEFAULT '';
    ALTER TABLE "SepetItems" ADD COLUMN IF NOT EXISTS "HediyePaketAdiEn" character varying(150) NOT NULL DEFAULT '';
    ALTER TABLE "SepetItems" ADD COLUMN IF NOT EXISTS "HediyePaketAdiAr" character varying(150) NOT NULL DEFAULT '';
    ALTER TABLE "SiparisDetaylari" ADD COLUMN IF NOT EXISTS "HediyePaketSecenegiId" integer NULL;
    ALTER TABLE "SiparisDetaylari" ADD COLUMN IF NOT EXISTS "HediyePaketAdi" character varying(150) NOT NULL DEFAULT '';
    ALTER TABLE "SiparisDetaylari" ADD COLUMN IF NOT EXISTS "HediyePaketAdiEn" character varying(150) NOT NULL DEFAULT '';
    ALTER TABLE "SiparisDetaylari" ADD COLUMN IF NOT EXISTS "HediyePaketAdiAr" character varying(150) NOT NULL DEFAULT '';

    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'FK_SepetItems_HediyePaketSecenekleri') THEN
        ALTER TABLE "SepetItems" ADD CONSTRAINT "FK_SepetItems_HediyePaketSecenekleri" FOREIGN KEY ("HediyePaketSecenegiId") REFERENCES "UrunHediyePaketSecenekleri" ("Id") ON DELETE SET NULL;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'FK_SiparisDetaylari_HediyePaketSecenekleri') THEN
        ALTER TABLE "SiparisDetaylari" ADD CONSTRAINT "FK_SiparisDetaylari_HediyePaketSecenekleri" FOREIGN KEY ("HediyePaketSecenegiId") REFERENCES "UrunHediyePaketSecenekleri" ("Id") ON DELETE SET NULL;
    END IF;

    CREATE INDEX IF NOT EXISTS "IX_UrunHediyePaketSecenekleri_UrunId_Sira" ON "UrunHediyePaketSecenekleri" ("UrunId", "Sira");
    CREATE INDEX IF NOT EXISTS "IX_SepetItems_HediyePaketSecenegiId" ON "SepetItems" ("HediyePaketSecenegiId");
    CREATE INDEX IF NOT EXISTS "IX_SiparisDetaylari_HediyePaketSecenegiId" ON "SiparisDetaylari" ("HediyePaketSecenegiId");

    INSERT INTO "UrunHediyePaketSecenekleri" ("UrunId", "Ad", "AdEn", "AdAr", "Fiyat", "AktifMi", "Sira", "OlusturulmaTarihi", "SilindiMi")
    SELECT u."Id", 'Standart paket', 'Standard package', 'تغليف قياسي', GREATEST(0, u."HediyePaketFiyati"), true, 1, NOW(), false
    FROM "Urunler" u
    WHERE (u."HediyePaketiVarMi" OR u."HediyePaketFiyati" > 0)
      AND NOT EXISTS (SELECT 1 FROM "UrunHediyePaketSecenekleri" p WHERE p."UrunId" = u."Id");

    IF NOT EXISTS (SELECT 1 FROM "UrunHediyePaketSecenekleri" WHERE "UrunId" IS NULL AND NOT "SilindiMi") THEN
        INSERT INTO "UrunHediyePaketSecenekleri" ("UrunId", "Ad", "AdEn", "AdAr", "Fiyat", "AktifMi", "Sira", "OlusturulmaTarihi", "SilindiMi")
        VALUES 
            (NULL, 'Standart Paket', 'Standard Gift Wrap', 'تغليف قياسي', 5.00, true, 1, NOW(), false),
            (NULL, 'Lüks Kutu', 'Luxury Gift Box', 'صندوق هدايا فاخر', 15.00, true, 2, NOW(), false),
            (NULL, 'Özel Tasarım Paket', 'Premium Custom Packaging', 'تغليف مخصص مميز', 25.00, true, 3, NOW(), false);
    END IF;

    WITH first_package AS (
        SELECT DISTINCT ON ("UrunId") "Id", "UrunId", "Ad", "AdEn", "AdAr", "Fiyat"
        FROM "UrunHediyePaketSecenekleri" WHERE NOT "SilindiMi" ORDER BY "UrunId", "Sira", "Id"
    )
    UPDATE "SepetItems" s SET "HediyePaketSecenegiId" = p."Id", "HediyePaketAdi" = p."Ad", "HediyePaketAdiEn" = p."AdEn", "HediyePaketAdiAr" = p."AdAr", "HediyePaketFiyati" = p."Fiyat"
    FROM first_package p WHERE s."UrunId" = p."UrunId" AND s."HediyePaketi" AND s."HediyePaketSecenegiId" IS NULL;

    WITH first_package AS (
        SELECT DISTINCT ON ("UrunId") "Id", "UrunId", "Ad", "AdEn", "AdAr", "Fiyat"
        FROM "UrunHediyePaketSecenekleri" WHERE NOT "SilindiMi" ORDER BY "UrunId", "Sira", "Id"
    )
    UPDATE "SiparisDetaylari" s SET "HediyePaketSecenegiId" = p."Id", "HediyePaketAdi" = p."Ad", "HediyePaketAdiEn" = p."AdEn", "HediyePaketAdiAr" = p."AdAr"
    FROM first_package p WHERE s."UrunId" = p."UrunId" AND s."HediyePaketi" AND s."HediyePaketSecenegiId" IS NULL;
    ...
    ALTER TABLE "Urunler" ADD COLUMN IF NOT EXISTS "IsExchangeable" boolean NOT NULL DEFAULT true;
    ALTER TABLE "Urunler" ADD COLUMN IF NOT EXISTS "NoExchangeBadgeColor" text NOT NULL DEFAULT '#DC2626';
    ...
    ALTER TABLE "UrunSecenekleri" ADD COLUMN IF NOT EXISTS "RenkKodu" text NOT NULL DEFAULT '';
    ALTER TABLE "UrunSecenekleri" ADD COLUMN IF NOT EXISTS "OlcuBirimi" text NOT NULL DEFAULT '';

    ALTER TABLE "Urunler" ADD COLUMN IF NOT EXISTS "OneCikanEtiketRengi" text NOT NULL DEFAULT '#D6AB5B';
    ALTER TABLE "Urunler" ADD COLUMN IF NOT EXISTS "YeniUrunEtiketRengi" text NOT NULL DEFAULT '#B33A3A';
    ALTER TABLE "Urunler" ADD COLUMN IF NOT EXISTS "KampanyaEtiketRengi" text NOT NULL DEFAULT '#31543B';
    ALTER TABLE "Urunler" ADD COLUMN IF NOT EXISTS "IndirimEtiketRengi" text NOT NULL DEFAULT '#B86A2F';

    CREATE TABLE IF NOT EXISTS "UrunToptanFiyatKademeleri" (
        "Id" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        "UrunId" integer NOT NULL,
        "UrunSecenekId" integer NULL,
        "MinAdet" integer NOT NULL DEFAULT 1,
        ...
        "OlusturulmaTarihi" timestamp with time zone NOT NULL DEFAULT now(),
        "SilindiMi" boolean NOT NULL DEFAULT false,
        CONSTRAINT "FK_UrunToptanFiyatKademeleri_Urunler_UrunId" FOREIGN KEY ("UrunId") REFERENCES "Urunler" ("Id") ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS "SosyalMedyaLinkleri" (
    "Id" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    "PlatformAdi" character varying(100) NOT NULL DEFAULT '',
    "Url" character varying(500) NOT NULL DEFAULT '',
    "IkonSinifi" character varying(100) NOT NULL DEFAULT '',
    "Sira" integer NOT NULL DEFAULT 0,
    "AktifMi" boolean NOT NULL DEFAULT true,
    "OlusturulmaTarihi" timestamp with time zone NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS "IX_SosyalMedyaLinkleri_Sira" ON "SosyalMedyaLinkleri" ("Sira");

-- Eski SiteAyarlari'ndaki sosyal medya URL'lerini taşı (yalnızca tablo boşsa)
INSERT INTO "SosyalMedyaLinkleri" ("PlatformAdi", "Url", "IkonSinifi", "Sira", "AktifMi", "OlusturulmaTarihi")
SELECT p.platform, p.url, p.ikon, p.sira, true, now()
FROM (
    (SELECT 'Facebook'    AS platform, s."FacebookUrl"   AS url, 'fab fa-facebook-f'  AS ikon, 1 AS sira FROM "SiteAyarlari" s WHERE COALESCE(s."FacebookUrl",  '') <> '' LIMIT 1)
    UNION ALL
    (SELECT 'Instagram'   AS platform, s."InstagramUrl"  AS url, 'fab fa-instagram'   AS ikon, 2 AS sira FROM "SiteAyarlari" s WHERE COALESCE(s."InstagramUrl", '') <> '' LIMIT 1)
    UNION ALL
    (SELECT 'TikTok'      AS platform, s."TiktokUrl"     AS url, 'fab fa-tiktok'      AS ikon, 3 AS sira FROM "SiteAyarlari" s WHERE COALESCE(s."TiktokUrl",    '') <> '' LIMIT 1)
    UNION ALL
    (SELECT 'YouTube'     AS platform, s."YoutubeUrl"    AS url, 'fab fa-youtube'     AS ikon, 4 AS sira FROM "SiteAyarlari" s WHERE COALESCE(s."YoutubeUrl",   '') <> '' LIMIT 1)
    UNION ALL
    (SELECT 'Pinterest'   AS platform, s."PinterestUrl"  AS url, 'fab fa-pinterest-p' AS ikon, 5 AS sira FROM "SiteAyarlari" s WHERE COALESCE(s."PinterestUrl", '') <> '' LIMIT 1)
    UNION ALL
    (SELECT 'Twitter / X' AS platform, s."TwitterUrl"    AS url, 'fab fa-x-twitter'   AS ikon, 6 AS sira FROM "SiteAyarlari" s WHERE COALESCE(s."TwitterUrl",   '') <> '' LIMIT 1)
) p
WHERE NOT EXISTS (SELECT 1 FROM "SosyalMedyaLinkleri" LIMIT 1);
...
INSERT INTO ""__EFMigrationsHistory"" (""MigrationId"", ""ProductVersion"")
SELECT '20260801142311_AddWholesaleTiersAndGeneralVariantDimensions', '10.0.10'
WHERE NOT EXISTS (
    SELECT 1 FROM ""__EFMigrationsHistory""
    WHERE ""MigrationId"" = '20260801142311_AddWholesaleTiersAndGeneralVariantDimensions'
)
AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'UrunSecenekleri' AND column_name = 'Beden')
AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'UrunSecenekleri' AND column_name = 'Renk')
AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'UrunSecenekleri' AND column_name = 'RenkKodu')
AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'UrunSecenekleri' AND column_name = 'OlcuBirimi')
AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'UrunToptanFiyatKademeleri');

INSERT INTO ""__EFMigrationsHistory"" (""MigrationId"", ""ProductVersion"")
SELECT '20260801211451_AddProductBadgeColors', '10.0.10'
WHERE NOT EXISTS (
    SELECT 1 FROM ""__EFMigrationsHistory""
    WHERE ""MigrationId"" = '20260801211451_AddProductBadgeColors'
)
AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'Urunler' AND column_name = 'OneCikanEtiketRengi')
AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'Urunler' AND column_name = 'YeniUrunEtiketRengi')
AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'Urunler' AND column_name = 'KampanyaEtiketRengi')
AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'Urunler' AND column_name = 'IndirimEtiketRengi');

INSERT INTO ""__EFMigrationsHistory"" (""MigrationId"", ""ProductVersion"")
SELECT '20260820095553_AddMultipleGiftPackageOptions', '10.0.10'
WHERE NOT EXISTS (
    SELECT 1 FROM ""__EFMigrationsHistory""
    WHERE ""MigrationId"" = '20260820095553_AddMultipleGiftPackageOptions'
)
AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'UrunHediyePaketSecenekleri')
AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'SepetItems' AND column_name = 'HediyePaketSecenegiId')
...
INSERT INTO ""__EFMigrationsHistory"" (""MigrationId"", ""ProductVersion"")
SELECT '20260825164021_AddSosyalMedyaLinkleri', '10.0.10'
WHERE NOT EXISTS (
    SELECT 1 FROM ""__EFMigrationsHistory""
...
INSERT INTO ""__EFMigrationsHistory"" (""MigrationId"", ""ProductVersion"")
SELECT '20260915120603_AddDiscountEndDateToVariants', '10.0.10'
WHERE NOT EXISTS (
    SELECT 1 FROM ""__EFMigrationsHistory""
    WHERE ""MigrationId"" = '20260915120603_AddDiscountEndDateToVariants'
)
AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'UrunSecenekleri' AND column_name = 'IndirimBitisTarihi');
");
