...
INSERT INTO "__EFMigrationsHistory" ("MigrationId", "ProductVersion")
SELECT '20260712150132_AddCarkKazanimi', '8.0.4'
WHERE NOT EXISTS (SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20260712150132_AddCarkKazanimi')
  AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'CarkKazanimlari')
  AND EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'CarkKazanimlari' AND indexname = 'IX_CarkKazanimlari_AppUserId');
""";
...
BEGIN
    ...
    CREATE TABLE IF NOT EXISTS "CarkKazanimlari" (
        "Id" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        "AppUserId" text NOT NULL,
        "CarkOdulId" integer NOT NULL,
        "KuponId" integer NULL,
        "OlusturulmaTarihi" timestamp with time zone NOT NULL DEFAULT NOW(),
        "SilindiMi" boolean NOT NULL DEFAULT false,
        CONSTRAINT "FK_CarkKazanimlari_AspNetUsers_AppUserId" FOREIGN KEY ("AppUserId") REFERENCES "AspNetUsers" ("Id") ON DELETE CASCADE,
        CONSTRAINT "FK_CarkKazanimlari_CarkOdulleri_CarkOdulId" FOREIGN KEY ("CarkOdulId") REFERENCES "CarkOdulleri" ("Id") ON DELETE RESTRICT,
        CONSTRAINT "FK_CarkKazanimlari_Kuponlar_KuponId" FOREIGN KEY ("KuponId") REFERENCES "Kuponlar" ("Id") ON DELETE SET NULL
    );
    CREATE UNIQUE INDEX IF NOT EXISTS "IX_CarkKazanimlari_AppUserId" ON "CarkKazanimlari" ("AppUserId");
    CREATE INDEX IF NOT EXISTS "IX_CarkKazanimlari_CarkOdulId" ON "CarkKazanimlari" ("CarkOdulId");
    CREATE INDEX IF NOT EXISTS "IX_CarkKazanimlari_KuponId" ON "CarkKazanimlari" ("KuponId");
